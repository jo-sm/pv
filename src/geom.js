import { vec3, vec4, mat3, quat } from '@mapbox/gl-matrix';

// calculates the signed angle of vectors a and b with respect to
// the reference normal c.
export const signedAngle = (a, b, c) => {
  const vector = vec3.create();

  vec3.cross(vector, a, b);

  return Math.atan2(vec3.dot(vector, c), vec3.dot(a, b));
};

// calculate a vector orthogonal to an input vector
export const ortho = (out, vec) => {
...
export const axisRotation = (out, axis, angle) => {
...
export const cubicHermiteInterpolate = (out, p_k, m_k, p_kp1, m_kp1, t, index) => {
  var p = vec3.create();
  var tt = t * t;
...
export const catmullRomSplineNumPoints = (numPoints, subdiv, circular) => {
  if (circular) {
...
// tangents.
export const catmullRomSpline = (points, numPoints, num, strength, circular, float32BufferPool) => {
  ...
  var delta_t = 1.0 / num;
  var m_k = vec3.create();
  const m_kp1 = vec3.create(); // tangents at k-1 and k+1
  const p_k = vec3.create();
  const p_kp1 = vec3.create();
  const p_kp2 = vec3.create();
  const p_kp3 = vec3.create();
  let i;
  let j;
  let e;
...
export class Sphere {
  constructor(center = vec3.create(), radius = 1.0) {
    this._center = center;
    this._radius = radius;
  }
...
// direction.
export const buildRotation = (rotation, tangent, left, up, use_left_hint) => {
  if (use_left_hint) {
...
export const interpolateScalars = (values, num) => {
