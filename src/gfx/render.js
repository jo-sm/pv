import { vec3, vec4, mat3 } from '@mapbox/gl-matrix';
import { TubeProfile, ProtoSphere, ProtoCylinder } from './geom-builders';
import { TraceVertexAssoc, AtomVertexAssoc } from './vert-assoc';
import { interpolateColor } from '../color';
import MeshGeom from './mesh-geom';
import BillboardGeom from './billboard-geom';
import { buildRotation, catmullRomSpline, catmullRomSplineNumPoints, ortho, signedAngle } from '../geom';
...
var smoothStrandInplace = (p, from, to, length) => {
  var bf = vec3.create();
  var af = vec3.create();
  var cf = vec3.create();
...
var spheresForChain = function(meshGeom, vertAssoc, opts, chain) {
...
export const spheres = function(structure, gl, opts) {
...
var billboardedSpheresForChain = function(meshGeom, vertAssoc, opts, chain) {
  var color = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  var atomCount = chain.atomCount();
  var idRange = opts.idPool.getContinuousRange(atomCount);
  meshGeom.addIdRange(idRange);
...
export const billboardedSpheres = function(structure, gl, opts) {
...
var ballsAndSticksForChain = function(meshGeom, vertAssoc, opts, chain) {
...
export const ballsAndSticks = function(structure, gl, opts) {
...
var pointsForChain = function(lineGeom, vertAssoc, chain, opts) {
...
export const points = function(structure, gl, opts) {
  console.time('points');
  var vertAssoc = new AtomVertexAssoc(structure, true);
  opts.color.begin(structure);
...
var linesForChain = function(lineGeom, vertAssoc, chain, opts) {
  var mp = vec3.create();
  var clr = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  var lineCount = 0;
  var atomCount = chain.atomCount();
...
export const lines = function(structure, gl, opts) {
  console.time('lines');
  var vertAssoc = new AtomVertexAssoc(structure, true);
  opts.color.begin(structure);
  var lineGeom = new LineGeom(gl, opts.float32Allocator);
...
//--------------------------------------------------------------------------
export const lineTrace = function(structure, gl, opts) {
...
var slineMakeTrace = function(lineGeom, vertAssoc, va, opts, traceIndex, trace) {
  var posOne = vec3.create();
  var posTwo = vec3.create();
  var colorOne = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  var colorTwo = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  ...
  var steps = catmullRomSplineNumPoints(trace.length(),
...
export const sline = function(structure, gl, opts) {
  console.time('sline');
  opts.color.begin(structure);
  var vertAssoc =
...
export const trace = function(structure, gl, opts) {
  console.time('trace');
...
// bases.
var _addNucleotideSticks = function(meshGeom, vertAssoc, traces, opts) {
  var rotation = mat3.create();
  var up = vec3.create();
  var left = vec3.create();
  var dir = vec3.create();
...
export const cartoon = function(structure, gl, opts) {
...
export const surface = function(data, gl, opts) {
  var pos = vec3.create();
  var normal = vec3.create();
  var color = vec4.fromValues(0.8, 0.8, 0.8, 1.0);
  var offset = 0;
...
var _cartoonAddTube = function(vertArray, pos, left, ss, tangent, color, radius, first, opts, offset, objId) {
  var rotation = mat3.create();
  var up = vec3.create();
...
var _colorPosNormalsFromTrace = function(meshGeom, trace, colors, positions, normals, objIds, pool, opts) {
  var pos = vec3.create();
  var normal = vec3.create();
  var lastNormal = vec3.create();
...
// trace.
var _cartoonForSingleTrace = function(meshGeom, vertAssoc, trace, traceIndex, opts) {
...
var _renderSingleTrace = function(meshGeom, vertAssoc, trace, traceIndex, opts) {
  if (trace.length() === 0) {
    return;
  }
  var rotation = mat3.create();
  ...
  var up = vec3.create();
  var midPoint = vec3.create();
  var caPrevPos = vec3.create();
  var caThisPos = vec3.create();
  var colorOne = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  var colorTwo = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  var idRange = opts.idPool.getContinuousRange(trace.length());
  meshGeom.addIdRange(idRange);
